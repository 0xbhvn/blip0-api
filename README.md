...
Multi-tenant monitoring API built with FastAPI for the Blip0 blockchain monitoring platform.
...
Boilerplate documentation is available in the `docs/` folder for reference on using the underlying features.

---

## 0. About

**blip0-api** is the configuration management layer for the Blip0 blockchain monitoring platform. It provides a REST API for managing monitors, triggers, and configurations that are consumed by the Rust-based oz-multi-tenant monitor runtime.

### Key Features for Multi-Tenant Monitoring:

- **Multi-tenant isolation** with Row-Level Security (RLS)
- **Monitor configuration management** for blockchain monitoring
- **Trigger configuration** (Email, Webhook, Discord, Slack)
- **Redis caching layer** for high-performance configuration access by Rust monitor
- **Platform-managed networks and filter scripts**
- **User-managed monitors and triggers**
- **Write-through caching** to Redis for sub-millisecond access
...
- ⚡️ **FastAPI** - Fully async Python web framework
- 🚀 **Pydantic V2** - Data validation (5x-50x faster)
- 🗄️ **SQLAlchemy 2.0** - Async ORM with PostgreSQL
- 🔐 **JWT Authentication** - With refresh tokens
- 🏬 **Redis** - Caching and pub/sub for real-time updates
- 🚦 **ARQ** - Background job processing
- 🛡️ **Row-Level Security** - Tenant data isolation
